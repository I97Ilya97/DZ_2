...
bool TPark::addOrder(TPassenger * passenger, TTime * start, TTime * finish) {
	...
	if (!vehicle) {
		throw new std::string("Не удалось найти подходящую машину\n");
	}
...
bool TPark::removeOrder(TOrder * order) {
	...
	if (!order)throw new std::string("Мне передали нулевой указатель\n");
	bool retval = false;
	try {
		delete(order);
	}
...
#ifdef DEBUG
		std::cout << "Ошибка освобождения памяти\n";
#endif
	}
	return retval;
}
bool TPark::removeOrder(unsigned int uuid) {
	...
	if (!uuid)throw new std::string("Мне передали нулевой UUID\n");
	bool retval = false;
